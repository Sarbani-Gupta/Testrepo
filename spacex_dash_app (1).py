...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        data = spacex_df
        fig = px.pie(data, values='class', names='Launch Site', title='Total success launches per site')
        return fig
    else:
        ...
        fig = px.pie(data, values='class_count', names='class', title=f'Total success launches per site "{entered_site}"')
        return fig
...
def scatter_plot(entered_sit, entered_payload):
    min_value = entered_payload[0]
    max_value = entered_payload[1]
    data = spacex_df.loc[(spacex_df['payload Mass (kg)'] >= min_value) & (spacex_df['Payload Mass (kg)'] <= max_vlaue)
    ]
    if entered_site == 'ALL':
